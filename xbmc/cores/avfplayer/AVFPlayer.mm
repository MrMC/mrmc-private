...
#if defined(HAS_AVFPLAYER) && defined(TARGET_DARWIN_IOS)
...
*/
    //URL = [NSURL URLWithString:@"http://184.72.239.149/vod/smil:BigBuckBunny.smil/playlist.m3u8"];
...
void CAVFPlayer::Process()
{
  ...
  try
  {
    ...
    {
      CLog::Log(LOGINFO, "CAVFPlayer: Auto-selecting %s based on configured bandwidth.", selected.c_str());
        ...
        case AVFSTATE::SETUP:
        {
          ...
          dispatch_sync(dispatch_get_main_queue(),^{
            ...
            */
            ...
            NSURL *nurl =[NSURL URLWithString:[filePath stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];

            CGRect frame = CGRectMake(0, 0,
              g_xbmcController.view.frame.size.width,
              g_xbmcController.view.frame.size.height);
            m_avf_avplayer = [[AVPlayerLayerViewNew alloc] initWithFrameAndUrl:frame withURL:nurl];
          });
...
bool CAVFPlayer::WaitForReadyToPlay(int timeout_ms)
{
  ...
  {
    if (m_avf_avplayer.player.status == AVPlayerStatusReadyToPlay)
    {
     AVPlayerItem *thePlayerItem = m_avf_avplayer.player.currentItem;
