...
CDVDVideoCodecVTB::~CDVDVideoCodecVTB()
{
  Dispose();
  pthread_mutex_destroy(&m_queue_mutex);
}

bool CDVDVideoCodecVTB::Open(CDVDStreamInfo &hints, CDVDCodecOptions &options)
{
...
void CDVDVideoCodecVTB::Dispose()
{
  DestroyVTSession();
  if (m_fmt_desc)
    CFRelease(m_fmt_desc), m_fmt_desc = NULL;
...
void CDVDVideoCodecVTB::SetDropState(bool bDrop)
{
  m_DropPictures = bDrop;
}

int CDVDVideoCodecVTB::Decode(uint8_t* pData, int iSize, double dts, double pts)
{
  if (pData)
  {
    ...
    // submit for decoding
    OSStatus status = VTDecompressionSessionDecodeFrame(vt_session,
      sampleBuff, decoderFlags, (void*)frameInfo, &infoFlagsOut);
    if (status != noErr)
    {
    ...
    status = VTDecompressionSessionWaitForAsynchronousFrames(vt_session);
    if (status != noErr)
    {
...
void CDVDVideoCodecVTB::Reset(void)
{
  // flush decoder
  VTDecompressionSessionRef vt_session = (VTDecompressionSessionRef)m_vt_session;
...
bool CDVDVideoCodecVTB::GetPicture(DVDVideoPicture* pDvdVideoPicture)
{
...
bool CDVDVideoCodecVTB::ClearPicture(DVDVideoPicture* pDvdVideoPicture)
{
...
void CDVDVideoCodecVTB::DisplayQueuePop(void)
{
...
void CDVDVideoCodecVTB::CreateVTSession(int width, int height, CMVideoFormatDescriptionRef fmt_desc)
{
  ...
  const VTDecompressionOutputCallbackRecord outputCallback = { VTDecoderCallback, this };
...
void
CDVDVideoCodecVTB::DestroyVTSession(void)
{
  if (m_vt_session)
  {
    VTDecompressionSessionInvalidate((VTDecompressionSessionRef)m_vt_session);
    CFRelease((VTDecompressionSessionRef)m_vt_session), m_vt_session = NULL;
  }
}

void 
CDVDVideoCodecVTB::VTDecoderCallback(
  ...
  CMTime             presentationDuration)
{
  ...
  CDVDVideoCodecVTB *ctx = (CDVDVideoCodecVTB*)decompressionOutputRefCon;
