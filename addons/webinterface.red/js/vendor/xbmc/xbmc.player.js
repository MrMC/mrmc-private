...
(function (window) {
	...
	xbmc.player.prototype = {
		...
		'init': function(element, options) {
			...
			// callbacks
			var self = this;
			$.each(['onBeforeInit', 'onInit', 'onPlayerChange', 'onActionStop', 'onActionPlay', 'onActionPause', 'onActionNext', 'onActionPrev', 'onUpdate', 'onPlayerStatusUpdated', 'onActivePlayersUpdated', 'onPlayingItemUpdated'], function(i, name) {
				// configuration
			...
			$(this).bind('onPlayerStatusUpdated.internal', function(e, status, result, playerData, obj) {
				if (status == 'success') {
			...
			$(this).bind('onActivePlayersUpdated.internal', function(e, status, result, obj) {
		...
		/**
		 * Binds html GUI elements that should be updated/extend with player info
		 *
		 * @param dom-node	DOM node/jQuery with player controls
		 * @return void
		 */
		'bindGuiElements': function(element) {
			this.$gui = $.extend(this.$gui, element);
			this._updateGui(true);
		},
		...
		/**
		 * Tell player to seek to a certain position
		 *
		 * @param integer	id			The id of the player to control
		 * @param mixed		position	Seek position in percent, time object ({hours:0,milliseconds:0,seconds:0,minutes:0}) or keyword (smallforward,smallbackward,bigforwawrd,bigbackward);
		 * @return void
		 */
		'seekTo': function(id, position) {
			this._sendPlayerAction(id, 'Seek', {'value' : position});
		},

		/**
		 * Returns the metadata of the currently playing item
		 *
		 * @return object
		 */
		 ...
		 * @return void
		 */
		'_updateGui': function(force) {
			var p = this.getActivePlayer();
			if (p) {
				if (this.updateGui == false) {
					for (var type in this.options.selector.controls) {
						this.enableControl(type);
					}
					this.updateGui = true;
				}
				if ((p.playStateChanged || force) && this.$controls && this.$controls.length) {
				...
				if ((p.isPlaying || p.playStateChanged || force) && this.$gui && this.$gui.length) {
					xbmc.core.fillPlaceholders(this.$gui, p);
				}
			} else if (this.updateGui || force) {
				// no player anymore? no need to update player related GUI items any further
				this.updateGui = false;
				...
				if (this.$gui && this.$gui.length) {
					xbmc.core.fillPlaceholders(this.$gui, {});
				}
			}

			// fire event
			var e = $.Event("onUpdate");
			this.fire.trigger(e, [p, this.$gui, this]);
		},
		...
		/**
		 * Updates the data of the playing item
		 *
		 * @param object	callbacks	Optional object with callback function on either success or error
		 * @return void
		 */
		'_updatePlayingItem': function(callbacks) {
			var p = this.getActivePlayer();
			if (p && p.id > -1 && (this.itemData.playlistid != p.playlistid || this.itemData.position != p.position || p.restarted)) {
				xbmc.rpc.request({
					'context': this,
					...
					'success': function(data) {
						if (data && data.result && data.result.item) {
							this.itemData = $.extend(data.result.item, {'playlistid':p.playlistid, 'position':p.position});
						} else {
							this.itemData = {};
						}
						if (callbacks && typeof(callbacks.success) == 'function') callbacks.success(data, this);
						...
						// fire event
						var e = $.Event("onPlayingItemUpdated");
						this.fire.trigger(e, ['success', data, this]);
					},
					'error': function(data, error) {
						xbmc.core.displayCommunicationError();
			...
			if (p && p.id > -1) {
				...
				xbmc.rpc.request({
					...
					'success': function(data) {
						var state = 'error';
						if (data && data.result) {
							this.activePlayers[p.id] = $.extend(p, data.result, {
								isPlaying: data.result.speed != 0,
								isPaused: data.result.speed == 0,
								playStateChanged: p.speed != data.result.speed,
								restarted: data.result.percentage < p.percentage && data.result.percentage < 3
							});
							state = 'success';
						} else {
